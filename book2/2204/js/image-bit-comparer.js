...
// Setup sizing
ImageBitComparer.BASE_WIDTH = 330; // Cahnged from 450
ImageBitComparer.BASE_HEIGHT = 220; // Changed from 300
ImageBitComparer.scale_factor = 1;

ImageBitComparer.INITAL_IMAGES = [
                                  ['IMG_5035.jpg', 'Sončnica'],
                                  ['IMG_5264.jpg', 'Streha templja'],
                                  ['IMG_2223.jpg', 'Jezero na Novi Zelandiji'],
                                  ['faces.jpg', 'Obrazi'],
                                  ['IMG_8354.jpg', 'Roža'],
                                  ['IMG_6698.jpg', 'Vrtnica v snegu'],
                                  ['IMG_8061.jpg', 'Raček'],
                                  ['wedding-vehicle.jpg', 'Avtomobil'],
                                  ['IMG_2521.jpg', 'Strela'],
                                  ['IMG_7448.jpg', 'Baloni'],
                                  ['black-and-white.jpg', 'Črno-bela fotografija'],
                                  ['IMG_0775.jpg', 'Knjige']
                                ];

ImageBitComparer.colour_labels = ['Rdeča', 'Zelena', 'Modra'];
...
$(document).ready(function(){
	...
	$('#comparer').on('show.bs.modal', function (e) {
	    // Mode set in data-mode attribute MUST match equivalent
		// mode value here in image-bit-comparer.js
		ImageBitComparer.mode = $(e.relatedTarget).attr('data-mode');
		var title = 'Primerjalnik barvnih globin slik'; // Default title
		if (ImageBitComparer.mode == 'change-bits') {
			title = 'Vpliv različnega števila bitov za vsako barvo';
		}
...
function populateSelectOptions() {
  ...
  for (var i = 0; i < ImageBitComparer.INITAL_IMAGES.length; i++) {
    var file = 'foto/' + ImageBitComparer.INITAL_IMAGES[i][0]; // Changed path from 'img/' to 'foto/'
	  ...
	  */
	  switch (number_of_bits) {
		case 1:
			...
			break;
		case 2:
			var subtitle_text = number_of_bits + ' bita';
			break;
		case 3:
		case 4:
			var subtitle_text = number_of_bits + ' biti';
			break;
		default:
			var subtitle_text = number_of_bits + ' bitov';
			break;
	  }
