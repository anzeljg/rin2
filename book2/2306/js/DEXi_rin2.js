    ...
    if (!att.link && att.attributes.length==0) { // basic attribute
        switch(this.inputType) {
            case "radio":
                for(var i=0; i<att.scale.values.length; i++) {
                    inpHtml +=
                        '<input type="radio" name="'+att.id+'" id="'+att.id+'_element" value="'+i+'"'+selected(i==altVal,"checked")+'>'+
                        '<span class="DEXi_'+this.model.valClass(att,i)+'">'+this.model.valName(att,i)+'</span>';
                }
                if (this.allowAll) {
                 inpHtml += '<input type="radio" name="'+att.id+'" id="'+att.id+'_element" value="*"'+selected(altVal==-2,"checked")+'>*';
                }
                break;
            case "check":
                for(var i=0; i<att.scale.values.length; i++) {
                    inpHtml +=
                        '<input type="checkbox" name="'+att.id+'" id="'+att.id+'_element" value="'+i+'"'+selected(altVals.indexOf(i)>=0,"checked")+'>'+
                        '<span class="DEXi_'+this.model.valClass(att,i)+'">'+this.model.valName(att,i)+'</span>';
                }
                break;
            case "none":
                ...
                break;
            default: // select
                inpHtml = '<select name="'+att.id+'" id="'+att.id+'_element">';
                if (!this.allowAll) {
                    inpHtml +=
...
DEXiInterface.prototype.setupMenu = function() {
    ...
    jQuery("#DEXi_commands").append('<p>'+
        '<button type="button" id="btnData" class="btn btn-primary">Vnos podatkov</button> '+
        '<button type="button" id="btnEval" class="btn btn-primary">Vrednotenje</button> '+
        '<button type="button" id="btnReset" onclick="resetGraph()" class="btn btn-primary">Ponastavi</button> '+
        gap +
        '<button type="button" title="Skrèi drevo za eno stopnjo" id="btnShrink" class="btn btn-primary">&#x2196;</button> '+
        '<button type="button" title="Razširi drevo za eno stopnjo" id="btnExpand" class="btn btn-primary">&#x2198;</button> '+
		/*gap+
        '<br><i>Vnos</i>: '+
        '<span class="DEXi_menuitem">'+
        '<input type="radio" name="radEntry" value="select""/>Spustni seznam</input>&nbsp;'+
        '<input type="radio" name="radEntry" value="radio"/>Izbirni gumbi</input>&nbsp;'+
        '<input type="radio" name="radEntry" value="check"/>Potrditvena polja</input>&nbsp;'+
        '<input type="radio" name="radEntry" value="none"/>Brez</input>&nbsp;'+
        '</span>'+gap+
        '<br><span class="DEXi_menuitem">'+
        '<input type="checkbox" id="chkAllow"><i>Dovoli</i> * (vse vrednosti) </input>'+
        '</span>'+*/
        '</p>'
     );
...
jQuery(document).ready(function() {
    DEXi.initialize();
    intfc = new DEXiInterface(DEXi,"DEXi_model","indent");
